      ...
      <table id="users-table" class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nama Pengguna</th>
            <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
            <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Role</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr class="hover:bg-gray-50 transition-colors duration-150">
            ...
            <td class="px-4 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-sm text-gray-500">{{ $user->name }}</td>
            <td class="px-4 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-sm text-gray-500">{{ $user->email }}</td>
            <td class="px-4 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-sm text-gray-500">{{ $user->role }}</td>
            <td class="px-4 lg:px-6 py-3 lg:py-4 whitespace-nowrap text-center text-sm font-medium">
              <a href="{{ route('users.show', $user->slug) }}" class="text-green-600 hover:text-green-900 mx-1 sm:mx-2">
                <i class="fa fa-eye"></i>
              </a>

              @if (Auth::user()->role === 'admin')
              <a href="{{ route('users.edit', $user->slug) }}" class="text-indigo-600 hover:text-indigo-900 mx-1 sm:mx-2">
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
        <div class="flex items-start justify-between mb-2">
          ...
          <div class="flex items-center gap-3 ml-3">
            <a href="{{ route('users.show', $user->slug) }}" class="text-green-600 hover:text-green-900"><i class="fa fa-eye"></i></a>
            @if (Auth::user()->role === 'admin')
            <a href="{{ route('users.edit', $user->slug) }}" class="text-indigo-600 hover:text-indigo-900"><i class="fa fa-edit"></i></a>
            <button class="text-red-600 hover:text-red-900"><i class="fa fa-trash"></i></button>
            @elseif(Auth::user()->role === 'user' && Auth::user()->id === $user->id)
            <a href="{{ route('users.edit', $user->slug) }}" class="text-indigo-600 hover:text-indigo-900"><i class="fa fa-edit"></i></a>
            <button class="text-red-300 cursor-not-allowed" disabled><i class="fa fa-trash"></i></button>
            @else
            <button class="text-indigo-300 cursor-not-allowed" disabled><i class="fa fa-edit"></i></button>
            <button class="text-red-300 cursor-not-allowed" disabled><i class="fa fa-trash"></i></button>
            @endif
          </div>
        </div>
        <div class="text-xs text-gray-500">
          <span class="inline-block px-2 py-0.5 bg-gray-100 text-gray-700 rounded-full font-medium">{{ $user->role }}</span>
        </div>
      </div>
      @php $idx++; @endphp
      @endforeach
